...
export default function BeritaEdit({
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        form.post(`/dashboard/berita/${feed.id}`, {
            forceFormData: true,
            onSuccess: () => {
                toast.dismiss();
...
fileInputRef.current.value = '';
    ...
    // Tampilkan: preview baru (jika ada) > gambar existing > placeholder
    const existingImage = feed.image_path
        ? feed.image_path.startsWith('http')
            ? feed.image_path
            : `/${feed.image_path.replace(/^\/+/, '')}`
        : null;
    const displayImage = previewUrl || existingImage || null;
                            ...
                            <div className="grid gap-3">
                                {displayImage ? (
                                    <div className="relative w-full overflow-hidden rounded-lg border">
                                        <img
                                            ...
                                            onError={(e) => {
                                                (e.target as HTMLImageElement).src =
                                                    'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22100%22 height=%22100%22%3E%3Crect width=%22100%25%22 height=%22100%25%22 fill=%22%23f1f5f9%22/%3E%3C/svg%3E';
                                            }}
                                        />
                                        {previewUrl && (
                                            <button
                                        ...
                                        {previewUrl && (
                                            <div className="absolute left-2 top-2 rounded-full bg-emerald-600 px-2 py-0.5 text-xs font-medium text-white">
                                                Gambar Baru
                                            </div>
                                        )}
                                    </div>
