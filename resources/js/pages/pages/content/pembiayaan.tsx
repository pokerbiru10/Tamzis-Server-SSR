...
import {
    pembiayaanPageDefaults
    
} from '@/content/pembiayaan-page-defaults';
import type {PembiayaanPageKey} from '@/content/pembiayaan-page-defaults';
...
interface PembiayaanContentProps {
    breadcrumbs?: Array<{ title: string; href: string }>;
    savedContents: Record<string, unknown>;
}

const pageKeys = Object.keys(pembiayaanPageDefaults) as PembiayaanPageKey[];
...
// Seksi generik: judul + intro/desc + daftar (string atau {name,desc}) + note,
// termasuk sub-seksi bersarang (mis. ilustrasi pada Griya Tumbuh Bahagia).
...
function PagePreview({
    ...
    return (
        <div className="space-y-5 bg-[#f8f9fa] p-4">
            ...
            >
                <div className="relative z-10 mx-auto flex flex-col items-center px-4">
                    ...
                    <nav className="flex items-center justify-center space-x-2 text-[9px] font-bold tracking-[0.2em] text-emerald-100/70 uppercase">
                        ...
                        <span>{breadcrumb.financing ?? breadcrumb.savings}</span>
                        <ChevronRight className="h-3 w-3" />
            ...
            {/* Seksi daftar (keunggulan, syarat, tujuan, akad, dll.) */}
...
export default function PembiayaanContentPage({
    savedContents,
}: PembiayaanContentProps) {
    const [drafts, setDrafts] = useState<Record<string, PageDraft>>(() =>
        Object.fromEntries(
            pageKeys.map((key) => [
                key,
                deepMergeContent(
                    pembiayaanPageDefaults[key].defaults,
                    savedContents?.[key],
                ) as PageDraft,
            ]),
        ),
    );
    const [activePage, setActivePage] = useState<PembiayaanPageKey>(
        pageKeys[0],
    );
    ...
    return (
        <>
            <Head title="Konten Halaman Pembiayaan" />
            ...
            <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-lg font-semibold">
                                Konten Halaman Pembiayaan
                            </h1>
                            <p className="text-sm text-muted-foreground">
                                Edit teks dan gambar halaman produk pembiayaan dengan preview langsung, seperti di WordPress.
                            </p>
                        </div>
                    </div>
                ...
                <div className="flex flex-wrap gap-2">
                    {pageKeys.map((key) => (
                        ...
                        >
                            {pembiayaanPageDefaults[key].name}
                        </button>
                    ))}
                </div>
                ...
                <div className="grid items-start gap-6 xl:grid-cols-2">
                    <div className="space-y-6">
                        ...
                        <Card>
                            <CardHeader>
                                <CardTitle>
                                    {pembiayaanPageDefaults[activePage].name} —{' '}
                                    {activeLocale === 'id'
                                        ? 'Bahasa Indonesia'
                                        : 'English'}
                                </CardTitle>
                        ...
                        <div className="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-800">
                            ...
                            <p>
                                Kotak &quot;Simulasi&quot; di halaman ini tetap otomatis mengarah ke kalkulator simulasi masing-masing produk, tidak diedit dari sini.
                            </p>
                        </div>
                        ...
                        <div className="flex justify-end">
                            ...
                            >
                                ...
                                {saving
                                    ? 'Menyimpan...'
                                    : `Simpan ${pembiayaanPageDefaults[activePage].name}`}
                            </Button>
                        </div>
                    </div>
